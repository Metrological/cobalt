...
"""Wpe implementation of Starboard launcher abstraction."""
...
class Launcher(abstract_launcher.AbstractLauncher):
  ...
  def __init__(self, platform, target_name, config, device_id, **kwargs):
    ...
    if not self.device_id:
      self.device_id = self.full_env.get('WPE_ADDR')
      if not self.device_id:
        raise ValueError(
            'Unable to determine target, please pass it in, or set WPE_ADDR '
  ...
  def _InitPexpectCommands(self):
    ...
    wpe_user_hostname = Launcher._WPE_USERNAME + '@' + self.device_id
    wpe_test_path = os.path.join(test_base_dir, test_file)

    # rsync command setup
    options = '-avzLh --exclude obj/ --exclude obj.host/ --exclude gen/'
    source = test_dir_path
    destination = wpe_user_hostname + ':~/'
    self.rsync_command = 'rsync ' + options + ' ' + source + ' ' + destination

    # ssh command setup
    self.ssh_command = 'ssh ' + wpe_user_hostname
    ...
    # test command setup
    test_base_command = wpe_test_path + ' ' + escaped_flags
  ...
  def _PexpectSpawnAndConnect(self, command):
    """Spawns a process with pexpect and connect to the wpe.
    ...
    while True:
      try:
        ...
        if i == 0:
          ...
          self.pexpect_process.sendline(Launcher._WPE_PASSWORD)
          break
        else:
          # If any other input comes in, maybe we've logged in with rsa key or
          # wpe does not have password. Check if we've logged in by echoing
  ...
  def _CleanupPexpectProcess(self):
    ...
    if self.pexpect_process is not None and self.pexpect_process.isalive():
      # Send ctrl-c to the wpe and close the process.
      self.pexpect_process.sendline(chr(3))
      self.pexpect_process.close()

  def Run(self):
    """Runs launcher's executable on the target wpe.
